...
function testRanges(range1, range2) {
    if ( parseInt(range1[0]) <= parseInt(range2[0]) && parseInt(range1[1]) >= parseInt(range2[0]) ) {
        return 1;
    } else if ( parseInt(range2[0]) <= parseInt(range1[0]) && parseInt(range2[1]) >= parseInt(range1[0]) ) {
        return 1;
    } else {
        return 0;
    }
...
for (let i = 0; i < arr.length; i++) {
    ...
    count += testRanges(elfOne, elfTwo);
}
